...
export type Shape = { id: string; type: Exclude<Tool, 'select' | 'hand' | 'flowchart' | 'connect'> | 'terminator' | 'input-output'; x: number; y: number; w: number; h: number; stroke: string; fill: string; width: number; opacity: number; text: string; points?: string; dashed?: boolean; lineCap?: 'round' | 'butt' | 'square'; sloppiness?: 0 | 1 | 2 | 3 | 4 | 5; connection?: Connection; route?: { x: number; y: number }[]; flowKind?: FlowNodeKind }
export type DrawingStyle = Pick<Shape, 'stroke' | 'fill' | 'width' | 'opacity' | 'lineCap' | 'sloppiness'>
export const defaultStyle: DrawingStyle = { stroke: '#68618d', fill: 'none', width: 2, opacity: 100, lineCap: 'round', sloppiness: 0 }
...
export function readBoard(): { shapes: Shape[]; title: string } {
  try {
    ...
    if (saved && typeof saved.title === 'string' && Array.isArray(saved.shapes) && saved.shapes.every((s: Shape) => s && typeof s.id === 'string' && types.includes(s.type) && (s.flowKind === undefined || flowKinds.includes(s.flowKind)) && (s.lineCap === undefined || ['round', 'butt', 'square'].includes(s.lineCap)) && (s.sloppiness === undefined || Number.isInteger(s.sloppiness) && s.sloppiness >= 0 && s.sloppiness <= 5) && validRoute(s.route) && ['x', 'y', 'w', 'h', 'width', 'opacity'].every(k => Number.isFinite(s[k as keyof Shape])) && typeof s.text === 'string' && typeof s.stroke === 'string' && typeof s.fill === 'string')) {
